...
def plot_tensile_to_ppt(file_path, template_path=None, lines_per_graph=12, swap_xy=True, append_to_ppt=None, width_cm=15.0, height_cm=12.0):
    """拉伸报告Origin绘图并导出PPT"""
    import win32com.client
    import pythoncom
...
def plot_vda_to_ppt(file_path, template_path=None, lines_per_graph=12, swap_xy=True, width_cm=15.0, height_cm=12.0):
    """VDA报告Origin绘图并导出PPT（边绘图边导出OLE）"""
    import win32com.client
    import pythoncom
...
def plot_phase_change(file_paths, template_path=None, width_cm=11.0, height_cm=8.8):
    """相变点绘图：CSV文件，第3列X，第4列Y，每个文件一张图（边绘图边导出OLE）"""
    import win32com.client
    import pythoncom
    try:
        if op.oext:
    ...
    for i, file_path in enumerate(file_paths):
        ...
        copy_graph_to_ppt_ole(gname, prs, prs.Slides.Count, width_pt=width_pt, height_pt=height_pt, right_side=False)
        count += 1
        print(f"已完成第{i+1}/{len(file_paths)}张图表: {fname}")
